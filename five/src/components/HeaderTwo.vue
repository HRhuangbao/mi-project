<template>
  <!-- 头部2模块 -->
  <div class="HeaderTwo">
    <div class="HeaderTwo-top">
      <div class="HeaderTwo-top_left">
        <img
          @click="$router.back(-1)"
          src="data:image/png;base64,"
          alt
        />
      </div>
      <div class="HeaderTwo-top_middle">分类</div>
      <div class="HeaderTwo-top_right">
        <img
          src="data:image/png;base64,"
          alt
        />
      </div>
    </div>
  </div>
</template>
<script>
export default {};
</script>


<style lang="scss" scoped>
.HeaderTwo {
  width: 100%;
  box-sizing: border-box;

  .HeaderTwo-top {
    position: fixed;
    top: 0%;
    width: 100%;
    height: 1.5625rem;
    background: #f2f2f2;
    display: flex;
    align-items: center;
    justify-content: space-between;
    .HeaderTwo-top_left,
    .HeaderTwo-top_middle,
    .HeaderTwo-top_right {
      margin: 0px 0.2rem;
      width: 1rem;
      height: 1rem;
      color: #666;
      font-size: 0.45rem;
      text-align: center;
      line-height: 1rem;

      img {
        width: 1rem;
        height: 1rem;
      }
    }
  }
}
</style>